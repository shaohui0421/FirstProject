...
static void ui_dialog_adminpwd_hide()
{
...
static GtkWidget * mk_entry(int width, int height, char * text)
{
    ...
    g_signal_connect(G_OBJECT(entry_pwd),"focus",G_CALLBACK(ui_win_pop_up_keyboard), NULL);
    g_signal_connect(G_OBJECT(entry_pwd),"button_press_event",G_CALLBACK(ui_win_pop_up_keyboard), NULL);
...
static int ui_dialog_adminpwd_init(void)
{
    ...
    g_signal_connect (G_OBJECT (dialog), "realize",
                      G_CALLBACK (ui_compt_realize), &ui_dialog_adminpwd);
...
static void ui_dialog_adminpwd_show(void)
{
    if (verified) {
        ui_tab[UI_TYPE_DIALOG_CONFIG]->show();
    } else {
        if (dialog == NULL) {
            ui_dialog_adminpwd_init();
        }
...
static void ui_dialog_admin_adapt(void)
{
    if (dialog) {
        if (ui_dialog_adminpwd.status == UI_STATUS_SHOW) {
...
static void ui_dialog_adminpwd_destroy(void)
{
    if (dialog) {
...
struct ui_comp_s ui_dialog_adminpwd = {
    .type = UI_TYPE_DIALOG_ADMINPWD,
    .subtype = UI_TOPLEVEL_TYPE_DIALOG,
    .is_init = UI_NEED_INIT,
    .widget = &dialog,
    .init = ui_dialog_adminpwd_init,
    .show = ui_dialog_adminpwd_show,
    .hide = ui_dialog_adminpwd_hide,
    .destroy = ui_dialog_adminpwd_destroy,
    .adapt = ui_dialog_admin_adapt,
};
